...
//  Runner
//
//  Created by Rohan Dalvi on 8/9/20.
...
final class SignalingClient {
    ...
    func sendAnswer(rtcSdp: RTCSessionDescription, recipientClientId: String) {
        do {
            ...
            let msg = String(data: data, encoding: .utf8)!
            socket.write(string: msg)
            print("Send SDP answer message over to signaling: ", msg)
        } catch {
            print(error)
        }
    }
    ...
    func sendIceCandidate(rtcIceCandidate: RTCIceCandidate, master: Bool,
        ...
        do {
            debugPrint("Sending ICE candidates \(rtcIceCandidate)")
...
extension SignalingClient: WebSocketDelegate {
    ...
    func websocketDidReceiveMessage(socket: WebSocketClient, text: String) {
        ...
        if parsedMessage != nil {
            ...
            do {
                let jsonObject = try message.trimmingCharacters(in: NSCharacterSet.whitespacesAndNewlines).convertToDictionary()
